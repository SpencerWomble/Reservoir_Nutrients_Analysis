...
# ============================================================================
# Mapping mean NOx concentration by sampling station
# ----------------------------------------------------------------------------
# One dot per station, placed at its coordinates, with dot SIZE scaled to the
# station's average NOx across the full monitoring record. Bigger dot = higher
# average NOx.
#
# Spatial stack:
...
#   station id, longitude, latitude, and the NOx value.
#
...
nox_record<- data |>
  dplyr::mutate(characteristic = if_else(
    characteristic =="Inorganic nitrogen (nitrate and nitrite)", "nox", characteristic)) |> 
  dplyr::filter(characteristic == "nox") |> 
  dplyr::select(site, id, longitude, latitude, measure_value) |> 
  dplyr::rename(nox = measure_value)

head(nox_record)

# ---- 3. Collapse to one mean per station -----------------------------------
# Average NOx over the whole record, carrying the coordinates through.
# (Coordinates are constant within a station, so first() is just "keep it".)
#
# NOTE on censoring: this is a plain arithmetic mean. With left-censored NOx
...
# rest of the script does not care how mean_nox was produced.
station_means <- nox_record |>
...
#   geom_sf             -> your station dots, size mapped to mean_nox.
...
make_nox_map <- function(site_id, stations, buffer_m = 5000) {
  ...
  ggplot() +
    ...
    scale_size_area(max_size = 14, name = expression(Mean~NO[x]~(mg~L^-1))) +
    ...
    coord_sf(crs = 4326) +
    labs(
      title    = paste0("Average NOx concentration by sampling station: ", pretty_name),
...
site_ids <- unique(nox_record$site)
nox_maps <- map(set_names(site_ids), make_nox_map, stations = stations_sf)

nox_maps$center_hill
nox_maps$dale_hollow
nox_maps$percy_priest
...
#ggsave("nox_station_map.pdf", nox_map, width = 8, height = 6.5)
#ggsave("nox_station_map.png", nox_map, width = 8, height = 6.5, dpi = 300)


# ============================================================================
# Optional tweaks
# ----------------------------------------------------------------------------
# Redundant colour encoding (area + colour both from mean_nox). Some find this
# easier to read; drop it in by adding aes(fill = mean_nox) and replacing the
# fixed fill with a scale:
#
#   geom_sf(aes(size = mean_nox, fill = mean_nox), shape = 21,
...
# maps without the scaled point sizes
...
make_nox_map <- function(site_id, stations, buffer_m = 5000) {
  ...
  ggplot() +
    ...
    geom_sf(
      data   = sites,
      size   = 4,          # fixed point size, not tied to any variable
      shape  = 21,
      fill   = "#1f6f78",
      colour = "white",
      stroke = 0.4,
    ...
    coord_sf(crs = 4326) +
    labs(
      title    = paste0("Sampling station locations: ", pretty_name),
      caption  = "Hydrography: USGS NHDPlus"
    ) +
    theme_minimal(base_size = 12) +
    theme(
